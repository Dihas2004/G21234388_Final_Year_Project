...
function HomePage() {
  // State hooks to manage selected files, prediction variable, and loading state
  const [selectedFiles, setSelectedFiles] = useState([]);
  const [predictionVar, setPredictionVar] = useState("");
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  // Handle file input changes by converting the file list to an array and updating state
  ...
  // Handle form submission: validate inputs, send files and variable to backend, and save results
  const handleSubmit = async () => {
    // Validate that files are selected and prediction variable is provided
    ...
    setLoading(true);
    // Build form data with files and prediction variable
    const formData = new FormData();
    selectedFiles.forEach((file) => formData.append("files", file));
    formData.append("prediction_variable", predictionVar);

    try {
      // Use environment variable for API URL; default to localhost if not provided
